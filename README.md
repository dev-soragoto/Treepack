...
```powershell
treepack [-c kit.toml] [-s DIR] [-o DIR] [--work-dir DIR] [--keep-work] [-p PROXY] [--download-retries N] [--github-token TOKEN] [-v] [-h]
```
...
只有在 `kit.toml` 的 `[build]` 中声明 `archive` 时才会创建 zip。省略 `archive` 会只生成输出目录和报告，适合 GitHub Actions 等 CI 交给上传步骤自动打包的场景。
